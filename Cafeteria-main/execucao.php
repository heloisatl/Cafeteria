...
function listarMenu($pedido) {
    ...
    foreach ($menu as $produto) {
        if ($produto instanceof Comida) {
            echo "\n ID: {$produto->getId()} - Nome: {$produto->getNome()} - Valor: R$ " . number_format($produto->getValor(), 2, ',', '.') . " - Tipo: {$produto->getTipo()} \n";
        }
    }
}
...
function calcularTotalPedido($pedido) {
    ...
    echo "O(a) Cliente {$pedido->getNomeCliente()} atendido pelo garçom {$pedido->getNomeGarcom()} e pelo caixeiro {$pedido->getNomeCaixeiro()}, comprou o(s) produto(s): ";
